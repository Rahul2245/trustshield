...
class OrganizationController {
    ...
    joinOrganization = async (req, res, next) => {
        try {
            ...
            const org = await organization_model_1.OrganizationModel.findByIdAndUpdate(req.params.id, { $addToSet: { members: userObjId } }, { new: true });
            if (!org) {
                res.status(404).json({ success: false, message: 'Organization not found' });
                return;
            }
    ...
    leaveOrganization = async (req, res, next) => {
        try {
            ...
            const orgCheck = await organization_model_1.OrganizationModel.findById(req.params.id);
            if (!orgCheck) {
                res.status(404).json({ success: false, message: 'Organization not found' });
                return;
            }
            // Owner cannot leave
            if (orgCheck.ownerId.equals(new mongoose_1.default.Types.ObjectId(userId))) {
                res.status(400).json({ success: false, message: 'Owner cannot leave. Transfer ownership first.' });
                return;
            }
            const userObjId = new mongoose_1.default.Types.ObjectId(userId);
            const org = await organization_model_1.OrganizationModel.findByIdAndUpdate(req.params.id, { $pull: { members: userObjId } }, { new: true });
            res.status(200).json({
                success: true,
