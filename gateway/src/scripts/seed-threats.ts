...
import { SecurityEventLogModel } from "../modules/admin/models/security-event-log.model";
import { AdminAlertModel } from "../modules/admin/models/admin-alert.model";
...
async function seedThreats() {
    ...
    await SecurityEventLogModel.deleteMany({});
    ...
    await SecurityEventLogModel.insertMany(logsToInsert);
    console.log(`Inserted ${logsToInsert.length} threat logs.`);

    await AdminAlertModel.insertMany(alertsToInsert);
